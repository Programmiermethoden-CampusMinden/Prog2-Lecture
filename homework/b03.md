...
-   Analysieren Sie die Vorgaben und erstellen Sie ein UML-Klassendiagramm, welches
    die Beziehungen zwischen den Klassen zeigt.
-   Ergänzen Sie das Spielmodell `GameEngine` und die Modellierung des Spielzustands
    ...
    ...)
-   Nutzen Sie das Observer-Pattern, so dass die GUI `GamePanel` automatisch
    aktualisiert wird, wenn sich der Spielzustand ändert.
-   Nutzen Sie das Observer-Pattern, so dass die `GameEngine` automatisch
