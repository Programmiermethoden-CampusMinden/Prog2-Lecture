...
2 Klassen:
- A nutzt B
- B ist noch nicht fertig (alle Methoden mit `throw new UnsupportedOperationException();`)
- A hat einige Methoden implementiert, andere noch nicht
- Methode in A soll getestet werden:
    -   Input-Parameter => ÄK/GW-Analyse
    -   Mocking für B, da keine Methoden implementiert
    -   Spy für A: implementierte Methoden nutzen, nicht implementierte mocken
- Enum zusätzlich für Beschränkung des Wertebereichs


Enum: RACE, SINGLE_SPEED, FIXIE, GRAVEL, EBIKE
Bicylce: gears(), changeGears(to), type() => Klasse in Record wandeln
Order: bicycle, customer, description => Klasse in Record wandeln
Shop: queue mit 5 plätzen, accept(order): nur wenn platz und nur eine pro kunde und nur RACE/SS/FIXIE, repair(): queue -> ready-list, deliver(kunde): aus ready-list
bike repair shop
Cycle Chronicles, Sprocket & Spokes, Pedal Pursuits
