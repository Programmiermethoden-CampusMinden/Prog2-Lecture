...
::: tip
**Hinweis**: Da wir hier keine Java-Module verwenden, müssen alle Klassen und
Interfaces, die in einer `sealed`-Hierarchie über `permits` verbunden sind, im
gleichen Package liegen, z.B. `zoo.animal`. Unterpakete wie `zoo.animal.birds` sind
deshalb für `sealed` hier nicht möglich.
:::
...
-   Kein Tier kann in einem Gehege mehrfach vorkommen. Lösen Sie dies durch die
    Auswahl einer geeigneten inneren Datenstruktur und begründen Sie Ihre Wahl.
...
-   sowie ein `CatHouse`, welches nur Objekte einer konkreten vom Interface `Cat`
...
-   `findEnclosureByName` gibt das erste gefundene Gehege mit diesem Namen aus dem
    Zoo zurück (oder `null`, wenn nicht gefunden)

-   `getAllAnimals` gibt eine Liste aller Tiere in allen Gehegen zurück

-   `getAllMammals` gibt eine Liste aller Tiere in allen Gehegen zurück, deren Typ
...
-   `getOvercrowdedEnclosures` gibt eine Liste aller Gehege zurück, die mehr Tiere
    enthalten, als der Parameter angibt

-   `String summary()` erstellt eine kurze textuelle Zusammenfassung des aktuellen
...
`Collectors.counting` o.ä. Wählen Sie die Parameter und Rückgabetypen geeignet und
...
Legen Sie dazu in `Zoo` einen Logger an und loggen Sie in den verschiedenen
Methoden. Nutzen Sie dabei

-   das Level `INFO` für die Ausführung der jeweiligen Methode,
-   das Level `FINE` für eine kurze Zusammenfassung des Zustands nach Ausführung der
    Methode (z.B. Anzahl Tiere im Zoo),
-   das Level `WARNING`, wenn ein angefordertes Tier oder Gehege nicht gefunden
    wird, und
