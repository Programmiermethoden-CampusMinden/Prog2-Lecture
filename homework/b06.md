...
::: tip
*Hinweis*: Implementieren Sie wie in der Vorlesung gezeigt einen zustandsbehafteten Visitor. Sie arbeiten entsprechend aktiv mit mehreren Stacks, beispielsweise `Deque<Expr>` und `Deque<Value>`. Prinzipiell ist der Ablauf pro Knoten ungefähr dieser: Sie besuchen rekursiv die Kinder eines Knotens, nehmen sich danach Objekte von den Stacks und verarbeiten diese Objekte und pushen das Ergebnis wieder auf den richtigen Stack.
:::
