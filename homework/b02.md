...
1.  (1P) Erstellen Sie ein UML-Klassendiagramm, welches die Strukturen in der Vorgabe
    verdeutlicht.

2.  (2P) Sie finden einige Interfaces, die das Interface `Animal` erweitern, beispielsweise
    `Fish`, `Reptile`, `Cat`, ...
...
3.  (2P) Um Gehege zu modellieren, erstellen Sie in eine generische Klasse `Habitat` mit einer
...
4.  (3P) Für die Repräsentation eines Zoologischen Gartens mit mehreren verschiedenen Gehegen
...
5.  (2P) Leiten Sie von `Zoo` eine nicht-generische Klasse `Aquarium` ab. Aquarien können nur
