...
Beachten Sie, dass beide Kommandos einen Typ-Parameter brauchen, der den Typ des zu bearbeitenden Tieres beschreibt. Sie müssen hier geeignet mit Wildcards und Bounds arbeiten, damit die Kommandos nur für die Tiere der Tier-Hiercharchie erlaubt sind. Beachten Sie auch, dass Ihre Commands von `Command` ableiten, und dass dort der Typ-Parameter ein *Gehege* beschreibt. Erinnern Sie sich an die *PECS*-Diskussion ...


### Aufgabe 2.3: Command-Manager mit Undo/Redo

Implementieren Sie nun die generische Klasse `CommandManager`. Der Typ-Parameter beschreibt einen Gehege-Typ.
...
Loggen Sie jeweils den Start der Methode, das Nichtausführen oder Scheitern einer Operation und am Ende der Methode den Zustand des Geheges `target`.

Demonstrieren Sie die Verwendung des `CommandManager` (beispielsweise in einem JUnit-Test), um eine Sequenz von
Zoo-Aktionen auszuführen und mit Undo/Redo zu experimentieren.

## Aufgabe 3: `Result<E,R>`

Definieren Sie sich analog zur Vorlesung einen Typ `Result<E,R>` und legen Sie sich ein Enum an für tpyische Fehlersituation im Zoo-Modell.
